...
const HODDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      else console.error('[HOD] pending failed:', pendingRes.reason?.message)
      if (allRes.status === 'fulfilled') setAllEvents(allRes.value.data)
      else console.error('[HOD] my-events failed:', allRes.reason?.message)
      if (statsRes.status === 'fulfilled') setStats(statsRes.value.data)
      else console.error('[HOD] stats failed:', statsRes.reason?.message)
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1">Review and approve departmental event requests</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600 dark:text-gray-400">Pending</p><p className="text-2xl font-bold text-yellow-600">{stats.pendingApprovals || 0}</p></div>
            <Clock className="w-10 h-10 text-yellow-600" />
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600 dark:text-gray-400">Approved</p><p className="text-2xl font-bold text-green-600">{stats.approved || 0}</p></div>
            <CheckCircle className="w-10 h-10 text-green-600" />
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-gray-600 dark:text-gray-400">Total Events</p><p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.totalEvents || 0}</p></div>
      ...
      {activeTab === 'pending' && (
        <div className="grid md:grid-cols-2 gap-4">
          {pendingEvents.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-400 col-span-2 py-8 text-center">No pending approvals</p>
      ...
      {activeTab === 'history' && (
        <HistorySection events={allEvents} role="hod" />
      )}
    </div>
  )
}
